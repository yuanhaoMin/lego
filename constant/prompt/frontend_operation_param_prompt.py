...
strategyUserId; string; null; 关联用户/人员Id; "200"
startTime; string; null; 有效时间开始/开始时间, 时间格式为HH:mm;
finishTime; string; null; 有效时间结束/结束时间, 时间格式为HH:mm;
...
"""
    + JSON_TEMPLATE_MIDDLE
    + """
Input: 策略名称是会议室区域策略, 设置推送用户是乾坤物联, 告警配置为弹出框告警, 设置策略行为是禁止进入, 告警级别是普通, 关联的用户是办公楼区, 设置有效区域为会议室, 有效时间是早上的8点开始到早上的9点结束, 重复日期为每周一到周五, 备注策略描述是会议室区域的策略, 特定时间段内禁止进入.
Output:
{
  "alarmUser": "151",
  "config": "2",
...
passDotCount; int; null; 停留时间几秒; 3
...
onceStartTime; string; null; 只在repeatType为0时(即不重复)有效, 开始时间, 时间格式为yyyy-MM-dd HH:mm:ss;
onceFinishTime; string; null; 只在repeatType为0时(即不重复)有效, 结束时间, 时间格式为yyyy-MM-dd HH:mm:ss;
startTime; string; null; 只在repeatType为1时(即重复)有效, 有效时间段的开始时间, 时间格式为HH:mm;
finishTime; string; null; 只在repeatType为1时(即重复)有效, 有效时间段的结束时间, 时间格式为HH:mm;
```
Here are some id and name information:
personId; personName
30; 张三
...
"""
    + JSON_TEMPLATE_MIDDLE
    + """
Input: 名称是夜检, 人员要张三和李四, 巡检区域是仓储和前台. 周一到周四每天晚上八点到九点半重复巡检, 停留时间设置十秒
Output:
{
  "patrolName": "夜检",
  "personIds": "30,41",
